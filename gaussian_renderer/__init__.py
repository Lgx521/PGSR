...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    input_all_map = None
    if return_plane:
        # 这个 all_map 的计算只在 return_plane=True 时需要
    ...
    # 调用 rasterizer，它现在总是返回6个值
    rendered_image, radii, out_observe, out_all_map, plane_depth, n_contrib = rasterizer(
        means3D = means3D,
        means2D = means2D,
    ...
    if not return_plane:
        return_dict =  {"render": rendered_image,
                        ...
                        "radii": radii,
                        "rasterizer_out": (out_observe, n_contrib) # 确保即使在这个分支也返回
                        }
    else:
        ...
        return_dict =  {"render": rendered_image,
                        ...
                        "rasterizer_out": (out_observe, n_contrib) 
                        }
    ...
    if return_plane and return_depth_normal:
